...
class IndexController extends Zend_Controller_Action {
	...
	public function indexAction() {
		...
		$this -> oCache = new Application_Model_Cache(md5('index/index'));
		if ($this -> oCache -> isOlderAs($this -> oSettings -> website -> cachetime)) {
			...
			array_push($aBoxes, $oPage -> getBox('<div id="wide_slide"><div id="wide_slide_slider" class="clearfix">' . $sWideImages . '</div></div>', 'slideshow', 'big_box'));
			...
			$this -> oCache -> writeCache($oPage -> getPage('door', $aNavigations, $aBoxes, 'index.js'));
	...
	public function contentpageAction() {
		...
		$this -> oCache = new Application_Model_Cache(md5('index/contentpage'));
